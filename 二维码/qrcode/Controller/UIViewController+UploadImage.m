...
- (void)headImageClick
{
    ...
    alertView.buttonTitles = [NSArray arrayWithObjects:@"拍照",@"从手机相册选取",@"取消", nil];
...
-(void)takePhoto
{
    if ( AVAuthorizationStatusDenied == [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo]) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:NSLocalizedString(@"Setting Permission for recording in Settings -> Privacy -> phone", nil) delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
        [alertView show];
        return;
    }
