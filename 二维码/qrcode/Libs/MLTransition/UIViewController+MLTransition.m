...
// add by huangyibiao
// 是否允许手势处理
static BOOL sg_canHandleGestureRegesture = YES;
...
{
    if (sg_canHandleGestureRegesture == NO) {
        return NO;
    }
    ...
    if (!vc.navigationController ||
        ...
        if ([UIDevice currentDevice].systemVersion.integerValue >= 7 && [vc.navigationController.transitionCoordinator isAnimated]) {
            return NO;
        } else {
            return YES;
        }
...
+ (void)setGestureEnabled:(BOOL)enabled {
    sg_canHandleGestureRegesture = enabled;
    return;
}
